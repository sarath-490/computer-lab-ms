    ...
    <nav className="bg-gradient-to-r from-indigo-600 to-purple-600 shadow-lg sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20">
          ...
          <div className="hidden md:flex items-center space-x-1">
            ...
            {isAuthenticated ? (
              <>
                <div className="ml-4 text-white px-4 py-2 flex items-center">
                  <User className="h-5 w-5 mr-2" />
                  <span className="truncate max-w-[120px]">{user?.name}</span>
                </div>
                <button
                  onClick={handleLogout}
                  className="ml-2 bg-white text-indigo-600 px-6 py-2 rounded-lg flex items-center font-medium hover:bg-indigo-50 hover:shadow-md transition duration-300 hover:scale-105"
                >
      ...
      <div className={`md:hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="px-4 pt-2 pb-4 space-y-2 bg-indigo-700/95 shadow-inner">
          ...
          {isAuthenticated ? (
            <>
              <div className="text-white px-4 py-3 flex items-center">
                <User className="h-5 w-5 mr-2" />
                <span className="truncate max-w-[200px]">{user?.name}</span>
              </div>
              <button
                onClick={handleLogout}
                className="bg-white text-indigo-600 w-full px-4 py-3 rounded-lg font-medium flex items-center justify-center hover:bg-indigo-50 transition duration-300 mt-2"
              >
                <LogOut className="h-5 w-5 mr-2" />
                Logout
              </button>
